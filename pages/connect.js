...
export default function ConnectPage() {
  ...
  // Hearts BG
  useEffect(() => {
    const cvs = document.getElementById("heartsCanvas"); if (!cvs) return;
    const ctx = cvs.getContext("2d"); if (!ctx) return;
    let W,H,rafId; const dpr = Math.max(1, window.devicePixelRatio || 1); let items = [];
    function resize(){ W = Math.round(innerWidth*dpr); H = Math.round(innerHeight*dpr);
    ...
    function spawn(){ const small=innerWidth<760; const size=(small?6:10)+Math.random()*(small?16:22);
      items.push({x:Math.random()*innerWidth,y:innerHeight+size,s:size,v:(small?0.5:0.9)+Math.random()*(small?0.6:0.9),
        c:["#ff6ea7","#ff8fb7","#ff4d6d","#e6007a"][Math.floor(Math.random()*4)]});}
    function draw(){ ctx.clearRect(0,0,W,H);
      items.forEach(h=>{ ctx.save(); ctx.globalAlpha=.9; ctx.translate(h.x,h.y); ctx.rotate(Math.sin(h.y/40)*.03);
        ctx.fillStyle=h.c; const s=h.s; ctx.beginPath(); ctx.moveTo(0,0); ctx.bezierCurveTo(s/2,-s,s*1.5,s/3,0,s);
        ctx.bezierCurveTo(-s*1.5,s/3,-s/2,-s,0,0); ctx.fill(); ctx.restore(); h.y-=h.v;});
      items=items.filter(h=>h.y+h.s>-40); if(Math.random()<(innerWidth<760?0.06:0.12)) spawn();
  ...
  function startFireworks(){
    const cvs=document.getElementById("fxCanvas"); if(!cvs) return; const ctx=cvs.getContext("2d");
    let W,H,ents=[]; function resize(){ W=cvs.width=innerWidth; H=cvs.height=innerHeight; }
    addEventListener("resize",resize); resize();
    function rand(a,b){return a+Math.random()*(b-a);}
    function hsv(h,s,v){ const f=(n,k=(n+h/60)%6)=>v-v*s*Math.max(Math.min(k,4-k,1),0);
    ...
    function tick(){ ctx.fillStyle="rgba(10,7,16,.22)"; ctx.fillRect(0,0,W,H);
  ...
  const handlePhotoPick = (e) => {
    try {
      ...
      r.onload = (ev) => {
        ...
        if (!du) return;
        const prev = Array.isArray(profile.photoDataUrls) ? profile.photoDataUrls : [];
        if (prev.length >= 3) { alert("Max 3 photos"); return; }
        const next = [...prev, du];
        const p = { ...profile, photoDataUrls: next };
        setProfile(p);
        try { localStorage.setItem("milan_profile", JSON.stringify(p)); } catch {}
      };
      r.readAsDataURL(f);
    } catch {}
  ...
  function startSearch(type){
    ...
    connectingRef.current=true; setIsSearching(true); setShowLoader(true);
    ...
    try{
      ...
      if(socketRef.current&&socketRef.current.off){
        socketRef.current.off("partnerFound"); socketRef.current.off("partnerDisconnected"); socketRef.current.off("connect_error");
      }
      ...
      socketRef.current.on("partnerFound",(data)=>{
        ...
        }catch{ setTimeout(()=>stopSearch(),500); }
      });
    ...
    }catch{ alert("Something went wrong starting the search."); stopSearch(); }
  ...
  useEffect(()=>{ function setCount(){ const w=innerWidth; setDiyaCount(w>1200?9:w>760?7:5); }
    setCount(); addEventListener("resize", setCount); return ()=>removeEventListener("resize", setCount); }, []);
    ...
    <>
      ...
      <aside className="sidebar">
        <div className="profileTop">
          ...
          <input id="photoPick" type="file" accept="image/*" style={{display:'none'}} onChange={handlePhotoPick}/>
        </div>
        <ul className="nav">
          <li>👤 Profile Info</li>
          <li>🔒 Security</li>
          <li>💘 Love Calculator</li>
          <li onClick={()=>{ try{ localStorage.clear(); }catch{} window.location.href='/login'; }}>🚪 Logout</li>
        </ul>
      </aside>
        ...
        <section className="featuresGrid" role="navigation" aria-label="Choose a mode">
          ...
          <article className="featureCard celebrate">
            ...
            <button className="cta gold" onClick={()=>{
              ...
              const x=innerWidth/2, y=innerHeight*0.58;
              for(let i=0;i<6;i++) setTimeout(()=>{ if(b) b(x+(Math.random()*260-130),y+(Math.random()*140-70)); }, i*120);
      ...
      <style>{`
        /* Hard reset helpers so external CSS can't break layout */
        :root{ --brandH: 170px; --bottomH: 120px; }
        *,*::before,*::after{ box-sizing: border-box; min-width:0; }
        ...
        /* Diyas — fixed bottom, behind content */
        ...
        /* ===== Mobile fixes ===== */
        @media(max-width:1024px){ :root{ --brandH: 160px; --bottomH: 110px; } .frame{ left:12px; right:12px; } .brandBlock{ top:118px; } .heroBrand{ font-size:96px; } }
        @media(max-width:860px){ :root{ --brandH: 150px; --bottomH: 120px; } .sidebar{display:none;} .frame{ left:12px; right:12px; } .heroWrap{ margin-left:0; } .featuresGrid{ width:min(980px, calc(100vw - 48px)); grid-template-columns:1fr 1fr; } }
        @media(max-width:560px){
          :root{ --brandH: 0px; --bottomH: 160px; }
          html,body{ overflow:auto; }  /* enable scroll on mobile */
          .brandBlock{ position:static; transform:none; top:auto; margin:16px 0 4px; pointer-events:none; text-align:center; }
          .heroBrand{ font-size:64px; }
          .brandTagline{ font-size:15px; }
          .heroWrap{ padding: 8px 12px var(--bottomH); justify-content:flex-start; gap:14px; }
