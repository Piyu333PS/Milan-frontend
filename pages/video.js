...
export default function VideoPage() {
  useEffect(() => {
    ...
    (async function start() {
      ...
      socket.on("disconnect", function (reason) { log("socket disconnected:", reason); });

      // create peer connection - stable transceivers + replaceTrack handling
      var createPC = function () {
        if (pc) return;
        log("creating RTCPeerConnection (stable transceivers)");
        pc = new RTCPeerConnection(ICE_CONFIG);

        // stable transceivers audio then video
        try {
          ...
          log("addTransceiver failed", e);
        }

        // attach local tracks by replacing if possible, else addTrack
        try {
          ...
          // find video sender: transceivers first
          ...
          if (!audioSender) {
            ...
            for (var jj = 0; jj < sList3.length; jj++) {
              var ss2 = sList3[jj];
              if (ss2 && ss2.track && ss2.track.kind === "audio") { audioSender = ss2; break; }
            }
          }
          ...
          if (localAudioTrack) {
            if (audioSender && typeof audioSender.replaceTrack === "function") {
              ...
              for (var kk = 0; kk < sList4.length; kk++) {
                var sss = sList4[kk];
                if (sss && sss.track && sss.track.kind === "audio") { hasAudioSender = true; break; }
              }
              if (!hasAudioSender) {
        ...
        pc.onicecandidate = function (e) {
          if (e && e.candidate) {
            try { socket.emit("candidate", e.candidate); } catch (ex) { log("emit candidate err", ex); }
          }
        };
        ...
        pc.onnegotiationneeded = async function () {
          try {
            ...
            try {
              var txt = String(err && err.name ? err.name : err || "");
              if (txt.indexOf("InvalidAccessError") !== -1 || /order of m-lines/i.test(String(err))) {
      ...
      socket.on("offer", async function (offer) {
        ...
        try {
          ...
          if (ignoreOffer) { log("Ignoring offer because not ready and not polite"); return; }
          await pc.setRemoteDescription(offerDesc);
          var answer = await pc.createAnswer();
          await pc.setLocalDescription(answer);
          socket.emit("answer", pc.localDescription);
          log("answer created & emitted");
        } catch (err) { console.error("Handling offer error:", err); }
      });
        ...
        try {
          ...
          screenTrack.onended = async function () {
            try {
              ...
              if (sender && cam) { await sender.replaceTrack(cam); showToast("Screen sharing stopped — camera restored"); }
