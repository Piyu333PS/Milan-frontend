  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      ...
      if (!searchLockedRef.current && !isAiPartnerRef.current) {
        if (!searchLockedRef.current && !isAiPartnerRef.current) { socket.emit("lookingForPartner", { type: "text" });

const onAiConnect = (payload = {}) => {
  console.log("ai:connected", payload);
...
// If already chatting with AI, ignore any non-AI partnerFound
if (isAiPartnerRef.current) {
  const incomingIsAI = partner?.isAI === true || partner?.type === "ai" || /Milan AI/i.test(partner?.name || "");
  if (!incomingIsAI) {
    try { socket.emit("declineHumanWhileAI"); } catch {}
    console.log("Ignored human partnerFound because AI session is locked.");
    return;
  }
}
if (isAiPartnerRef.current || searchLockedRef.current) {
      ...
      if (isAI) { searchLockedRef.current = true; setIsConnected(true); if (!rc) rc = `ai-room-${Date.now()}`; }// ✅ FIX 3: Properly lock search when AI connects
      if (isAI) { 
        searchLockedRef.current = true;
